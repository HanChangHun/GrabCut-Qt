...
class MainDialog(QDialog):
    def __init__(self):
        ...
        self.iteration_number_spin.valueChanged.connect(self.spinBoxChanged)
    ...
    def btn_clicked(self, state, btn):
        if btn.text() == 'open':
            ...
            cv2.imwrite("temp_result.jpg", self.segmented_image)
            cv2.waitKey(0)
    ...
    def spinBoxChanged(self):
        self.scribble_size = self.scribble_size_spin.value()
        self.iteration_number = self.iteration_number_spin.value()
        print(self.scribble_size, self.iteration_number)
